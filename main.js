function init() {
  ...
  $(".Graphic").append($(gui.domElement)); //Tạo bảng điều khiển các giá trị thay đổi
  //thêm vào class Glaphic trong thẻ div trong html
  ...
  Container.add(ObjectOne); //thêm bề mặt vào Container
  scene.add(Container); // sau đó thêm container vào Scene
  ...
  // biến đổi y để quay vật thể trong  animate 1
  animation1.add(
    {
  ...
  var plane1 = getPlane1(20); //tạo mặt đất
  plane1.name = "plane1";
  plane1.rotation.x = Math.PI / 2;
  plane1.position.y = -ObjectOne.geometry.parameters.height / 2;
  scene.add(plane1); //thêm mặt đất vào scene , mặt đất này ko có ánh sáng
  ...
  pointLight.visible = false; //set ánh sáng ban đầu chưa hiện , chỉ hiện sau khi kích hoạt
  scene.add(pointLight);

  var SpotLight = getSpotLight(8);
  SpotLight.name = "Spotlight";
  SpotLight.visible = false; //set ánh sáng ban đầu chưa hiện , chỉ hiện sau khi kích hoạt
  scene.add(SpotLight);

  var DirectionalLight = getDirectionalLight(8);
  DirectionalLight.name = "Directionallight";
  DirectionalLight.visible = false; //set ánh sáng ban đầu chưa hiện , chỉ hiện sau khi kích hoạt
  scene.add(DirectionalLight);

  var AmbientLight = getAmbientLight(10);
  AmbientLight.name = "Ambientlight";
  AmbientLight.visible = false; //set ánh sáng ban đầu chưa hiện , chỉ hiện sau khi kích hoạt
  scene.add(AmbientLight);

  //Add light controller for GUI
  PointlightGUI = gui.addFolder("PointLight"); //thêm folder điều khiển ánh sáng Pointlight (ở trên thanh navigation)
  ...
  PointlightGUI.addColor(point_parameters, "color")
    .onChange(function () {
  ...
  //======
  SpotlightGUI = gui.addFolder("SpotLight"); //thêm folder điều khiển ánh sáng Spotlight (ở trên thanh navigation)
  ...
  SpotlightGUI.addColor(spot_parameters, "color")
    .onChange(function () {
  ...
  SpotlightGUI.hide();
  //=================
  AmbientLightGUI = gui.addFolder("Ambientlight"); //thêm folder điều khiển ánh sáng Ambientlight (ở trên thanh navigation)
  ...
  DirectionalLightGUI = gui.addFolder("DirectionalLight"); //thêm folder điều khiển ánh sáng DirectionalLight (ở trên thanh navigation)
  DirectionalLightGUI.add(DirectionalLight, "intensity", 0, 20).name(
    "intensity"
  );
  ...
  DirectionalLightGUI.addColor(directional_parameters, "color")
    .onChange(function () {
  ...
  // set up thông số ban đầu điều khiển camera vào trong bảng công cụ GUI phía bên phải
  ...
  //Responsive màn hình của web
  window.addEventListener("resize", function () {
    var width = window.innerWidth; //lấy width của web
    var height = window.innerHeight; //lấy height của web
    renderer.setSize(width, height);
    camera.aspect = width / height; //tỉ lệ khung máy ảnh
    camera.updateProjectionMatrix(); //sau khi thay đổi các giá trị thì hàm này dùng để update lại
  });
  ...
  renderer.setClearColor("rgb(30, 0, 30)"); //xóa màu nền chuyển thành màu khác
  document.getElementById("webgl").appendChild(renderer.domElement);
  renderer.render(scene, camera);

  //Interactive camera by mouse
  var controls = new THREE.OrbitControls(camera, renderer.domElement); //thêm điều khiển chuột kéo thả
  ...
  var TfProperty = {
    Translate: function () {
      transformControls.mode = "translate"; //di chuyển đồ vật
    },
    Rotate: function () {
      transformControls.mode = "rotate"; //xoay đồ vật
    },
    Scale: function () {
      transformControls.mode = "scale"; //phóng to thu nhỏ đồ vật
    },
    Reset: function () {
      ResetObject(); // reset về vị trí ban đầu
    },
  };
  var TfMode = gui.addFolder("Transform Mode"); //thêm folder Transform vào GUI
...
function getDirectionalLight(intensity) {
  ...
  var directionalSphere = new THREE.Mesh(
    new THREE.OctahedronGeometry(1, 0),
    new THREE.MeshBasicMaterial({ color: 0xffff00 })
  );
      ...
      case 4:
        ...
        input.onchange = (e) => {
          ...
          reader.addEventListener(
            "load",
            function () {
              ObjectNew = new THREE.Mesh(
                ObjectOld.geometry,
                getMaterial("Texture", reader.result)
              );
              loadNew(ObjectNew, ObjectOld);
            },
            false
